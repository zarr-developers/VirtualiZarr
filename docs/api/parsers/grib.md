...
The [gribberish](https://github.com/mpiannucci/gribberish) library (a Rust-backed GRIB1/GRIB2 reader) provides a `GribberishParser` for VirtualiZarr. Each GRIB message becomes a single chunk, and reading a chunk back decodes the referenced message bytes through gribberish's registered zarr codec.
