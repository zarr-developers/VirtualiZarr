...
- **Recognized format** - Firstly, there must be a virtualizarr parser that understands how to parse the file format that your data is in. The VirtualiZarr package ships with parsers for a number of common formats, but if your data is not supported you may first have to write your own dedicated virtualizarr parser which understands your format (see the page on [Custom Parsers](custom_parsers.md)).
