...
- **Homogeneous dtypes** - The zarr data model assumes that every chunk of data in a single array decodes to the same dtype. For multi-file datasets each chunk often corresponds to (part of) one file, so if all your files do not have consistent compression or other codecs your data cannot be virtualized. This is arguably inherent to the concept of what an array is.
