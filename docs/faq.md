...
- **Homogeneous CF encoding** - Many tools (including Xarray) apply an additional decoding step upon opening which interprets the values of specific attribute fields such as `scale_factor` and `add_offset`, and applies additional decoding. As this decoding is applied per-variable, but Xarray/VirtualiZarr's default attribute concatenation behaviour is to merge all attributes by overwriting, concatenating two virtual datasets created from netCDF files with different values of `scale_factor` and `add_offset` can create a result which is silently decoded incorrectly by Xarray! We know this is a major footgun, see [issue #1004](https://github.com/zarr-developers/VirtualiZarr/issues/1004) for more details.
