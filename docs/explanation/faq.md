...
- **Storage compactness** - While Kerchunk JSON is extremely inefficient in its use of storage space, Kerchunk Parquet is actually very efficient, creating Parquet files that take up about ~3x less space on disk than Icechunk (v2.0's) manifest files do. In practice this is dwarfed by the storage space required for the actual data, but altough future versions of Icechunk could improve on this, for now Kerchunk Parquet is strictly more compact in this sense.
