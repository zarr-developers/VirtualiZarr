...
- **Homogeneous chunk shapes** - The zarr data model traditionally assumes that every chunk of data in a single array has the same chunk shape. For multi-file datasets each chunk often corresponds to (part of) one file, so if all your files do not have consistent chunking your data could not previously be virtualized. This restriction is now relaxed for concatenation: if your files have genuinely different (but internally uniform) chunk sizes along the concatenation axis, VirtualiZarr can combine them into a single **rectilinear** (variable-length) chunk grid - see [Rectilinear chunk grids](data_structures.md#rectilinear-chunk-grids). This support is still experimental and opt-in, and doesn't yet extend to appending to an existing rectilinear-chunked array in Icechunk or to region writes.
