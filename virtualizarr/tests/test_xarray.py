...
def test_xarray_concat_empty_data_var(tmp_path: Path):
    file_paths = [str(tmp_path / "tmp1.nc"), str(tmp_path / "tmp2.nc")]
    datasets = [
        xr.Dataset.from_dict(
            {
                "coords": {
                    ...
                    "x": {
                        "dims": ("x",),
                        "attrs": {},
                        "data": [-1996000.0, -1992000.0, -1988000.0, -1984000.0],
                    },
                    "y": {
                        "dims": ("y",),
                        "attrs": {},
                        "data": [-2495999.75, -2491999.75, -2487999.75, -2483999.75],
                    },
                    "time": {
                        "dims": ("time",),
                        "attrs": {},
                        "data": [datetime.datetime(2022, 12, 31, 12, 0)],
                    },
                },
                "attrs": {},
                "dims": {"time": 1, "y": 4, "x": 4},
                "data_vars": {
                    ...
                    "Lambert_Conformal": {"dims": (), "attrs": {}, "data": np.nan},
                },
            }
        ),
        xr.Dataset.from_dict(
            {
                "coords": {
                    ...
                    "time": {
                        "dims": ("time",),
                        "attrs": {},
                        "data": [datetime.datetime(2022, 12, 31, 13, 0)],
                    },
                },
                "attrs": {},
                "dims": {"time": 1, "y": 4, "x": 4},
    ...
    for fp, ds in zip(file_paths, datasets):
        ds.to_netcdf(fp)
    expected = xr.concat(datasets, dim="time", coords="minimal", compat="override")
    parser = HDFParser()
    ...
    virtual_datasets = [
        open_virtual_dataset(
            ...
            parser=parser,
            object_store=store,
            loadable_variables=["lat", "lon", "x", "y", "time"],
        )
        for url in file_paths
    ]
    observed = xr.concat(
        virtual_datasets, dim="time", coords="minimal", compat="override"
    )
    xr.testing.assert_identical(observed.coords, expected.coords)
