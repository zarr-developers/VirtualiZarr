...
def replace_virtual_with_loadable_vars(
    ...
    if isinstance(loadable_variables, list):
        ...
        var_names_to_load: list[str] = [name for name, var in loadable_ds.variables.items() if var.dims == (name,)]
    else:
        raise ValueError(f"loadable_variables must be an iterable of string variable names, or None, but got type {type(loadable_variables)}")

    print(loadable_variables)
    print(loadable_ds)
    # this will automatically keep any IndexVariables needed for loadable 1D coordinates
    ds_loadable_to_keep = loadable_ds[var_names_to_load]
